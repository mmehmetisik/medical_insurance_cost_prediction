...
###################
# Hedef Değişken Analizi (categorik)
###################
...
# 1. Risk Skoru (Sigara ve BMI kombinasyonu)
...
# 3. BMI ve Yaş Etkileşimi
df['age_bmi_interaction'] = (df['age'] * df['bmi']) / 100

# 4. Sigara-BMI Etkileşimi
df['smoker_bmi_interaction'] = df.apply(lambda x: x['bmi'] if x['smoker'] == 'yes' else 0, axis=1)

# 5. Sigara-Yaş Etkileşimi
df['smoker_age_interaction'] = df.apply(lambda x: x['age'] if x['smoker'] == 'yes' else 0, axis=1)

# 6. Genel Sağlık Skoru
df['health_score'] = df.apply(lambda x: (x['bmi'] * x['age'] * (3 if x['smoker'] == 'yes' else 1)) / 100, axis=1)

# Değişkenleri kontrol edelim
print("\nYeni değişkenlerin ilk birkaç satırı:")
new_features = ['risk_score', 'family_size', 'has_children', 'age_bmi_interaction',
...
cat_cols = ['sex', 'smoker', 'region', 'risk_score', 'has_children']

# Numerik değişkenlerimiz
num_cols = ['age', 'bmi', 'children', 'charges', 'family_size', 'age_bmi_interaction', 'smoker_bmi_interaction', 'smoker_age_interaction', 'health_score']
...
# Ridge için parametre grid'i
# Ridge için güncellenmiş parametre grid'i
# Ridge için güncellenmiş parametre grid'i
ridge_params = {
...
# GBM için parametre grid'i aynı kalabilir
gbm_params = {
...
# En iyi Ridge parametreleri ile final modeli oluşturalım
final_model = Ridge(
    alpha=0.1,  # Grid search'ten gelen en iyi alpha değeri gelecek
    fit_intercept=True,
    solver='auto',
    positive=False,
    random_state=17
...
def plot_importance(model, features, num=len(X), save=False):
    # Ridge için mutlak katsayıları kullan
...
feature_imp = plot_importance(final_model, X.columns)
